...
async function run(): Promise<void> {
  const config = TOML.parse(await fs.readFile(core.getInput('config'))) as Config;
  ...
  for(const cfg in config.configurations) {
    ...
    for(const target in config.targets) {
      ...
      const proc = childProcess.spawnSync(config.targets[target][0], config.targets[target].slice(1), {stdio: 'inherit'});
      core.endGroup();
    }
    core.endGroup();
